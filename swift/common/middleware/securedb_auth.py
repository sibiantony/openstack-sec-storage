# An authentication filter, based on the swift detault
# tempauth system. 
...
class SecAuth(object):
    """
    An authentication system based on tempauth to enfore secure
        storage.

    Add to your pipeline in proxy-server.conf, such as::

        [pipeline:main]
        pipeline = catch_errors cache secauth proxy-server
    ...
    And add a secauth filter section, such as::

        [filter:secauth]
        use = egg:swift#securedb_auth
        mdb_host = localhost
        mdb_user = openstack
        mdb_pass = openstack
        mdb_db   = swiftauth
    ...
    def __init__(self, app, conf):
        ...
        try:
            ...
            with db_con:
                dbcur = db_con.cursor()
                dbcur.execute("select account, uname, passwd, ugroups from user")
                dbusers = dbcur.fetchall()
    ...
    def handle_get_token(self, req):
        ...
        if not token:
            ...
            memcache_client.set(memcache_user_key, token,
                                timeout=float(expires - time()))
        # Add key to the db
        # Several ways to communicate to the application/service to supply the key
        #  - Probably requires a mapping from token -> user secret key
        #  - User secret key can be part of the token itself
        ...
        # Decrypt the account (tenant) key
        passwd_key = sha256(key).hexdigest()[:32]

        aes_dec_userkey = AES.new(passwd_key, AES.MODE_CFB)
        userkey = aes_dec_userkey.decrypt(enc_userkey)
        aes_dec_acckey = AES.new(userkey, AES.MODE_CFB)
        acckey = aes_dec_acckey.decrypt(enc_acckey)

        # Re-encrypt with the current token and store in db
        aes_enc_tokenkey = AES.new(token[-32:], AES.MODE_CFB)
        enc_tokenkey = aes_enc_tokenkey.encrypt(acckey)
        try:
            db_con = mdb.connect(self.db_host, self.db_user, self.db_pass, self.db_name)
            with db_con:
                dbcur = db_con.cursor()
